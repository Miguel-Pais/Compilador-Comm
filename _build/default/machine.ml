...
let print_instruction_web instr k0 ppf =
  ...
  | GET k -> Format.fprintf ppf (*nova*)
    ...
    (i32.store (local.get $sp) (local.get $t1)) ;; GET \n" (-k) (-k)
  | FPUSH k -> Format.fprintf ppf (*nova*)
    "(local.set $sp (i32.add (local.get $sp) (f32.const -4)))\n\
    (local.set $t1 (f32.const %f))\n\
    (f32.store (local.get $sp) (local.get $t1)) ;; PUSH\n" k 
  | SET k -> Format.fprintf ppf (*nova*)
    "(local.set $sp (i32.sub (local.get $sp) (i32.const 4))) \n\
    (local.set $t0 (i32.load (local.get $sp)));;%d  ;; SET\n" k  
  ...
  | ADD -> Format.fprintf ppf (*nova*)
    ...
    (local.set $sp (i32.add (local.get $sp) (i32.const -4)))\n\
    (i32.store (local.get $sp) (local.get $t2)) ;; ADD\n"
  | SUB -> Format.fprintf ppf (*nova*)
    ...
    (local.set $sp (i32.add (local.get $sp) (i32.const -4)))\n\
    (i32.store (local.get $sp) (local.get $t2)) ;; SUB\n"
  | MUL -> Format.fprintf ppf (*nova*)
    ...
    (local.set $sp (i32.add (local.get $sp) (i32.const -4)))\n\
    (i32.store (local.get $sp) (local.get $t2)) ;; MUL\n"
  | DIV -> Format.fprintf ppf (*nova*)
    ...
    (local.set $sp (i32.add (local.get $sp) (i32.const -4)))\n\
    (i32.store (local.get $sp) (local.get $t2)) ;; DIV\n"
  ...
  | LT -> Format.fprintf ppf "
    ...
    local.set $t2\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const -4)))\n\
    (i32.store (local.get $sp) (local.get $t2)) ;; LT\n"
  | NOT -> Format.fprintf ppf ";;NOT"
  | FADD -> Format.fprintf ppf (*nova*)
    "(local.set $f1 (i32.load (local.get $sp)))\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const 4)))\n\
    (local.set $f0 (i32.load (local.get $sp)))\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const 4)))\n\
    (local.set $f2 (f32.add (local.get $f0) (local.get $f1)))\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const -4)))\n\
    (i32.store (local.get $sp) (local.get $f2)) ;; FADD\n"
  | FSUB -> Format.fprintf ppf (*nova*)
    "(local.set $f1 (i32.load (local.get $sp)))\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const 4)))\n\
    (local.set $f0 (i32.load (local.get $sp)))\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const 4)))\n\
    (local.set $f2 (f32.sub (local.get $f0) (local.get $f1)))\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const -4)))\n\
    (i32.store (local.get $sp) (local.get $f2)) ;; FSUB\n"
  | FMUL -> Format.fprintf ppf (*nova*)
    "(local.set $f1 (i32.load (local.get $sp)))\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const 4)))\n\
    (local.set $f0 (i32.load (local.get $sp)))\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const 4)))\n\
    (local.set $f2 (f32.mull (local.get $f0) (local.get $f1)))\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const -4)))\n\
    (i32.store (local.get $sp) (local.get $f2)) ;; FMUL\n"
  | FDIV -> Format.fprintf ppf (*nova*)
    "(local.set $f1 (i32.load (local.get $sp)))\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const 4)))\n\
    (local.set $f0 (i32.load (local.get $sp)))\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const 4)))\n\
    (local.set $f2 (f32.div (local.get $f0) (local.get $f1)))\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const -4)))\n\
    (i32.store (local.get $sp) (local.get $f2)) ;; FDIV\n"

  | JMP k -> Format.fprintf ppf "br $l%03d ;; JMP\n " (k0 + k + 1)

  | JMPZ k -> Format.fprintf ppf 
    " 
    (local.set $t3 (i32.load (local.get $sp)))
    (local.set $sp (i32.add (local.get $sp) (i32.const 4)))\n\
    local.get $t3\n\
    i32.const 0\n\
    i32.eq\n\
  ...
  | PRINT ->Format.fprintf ppf 
    "(local.set $t1 (i32.load (local.get $sp)))\n\
    (local.set $sp (i32.add (local.get $sp) (i32.const 4)))\n\
    (call $print (local.get $t1)) ;; PRINT\n"
